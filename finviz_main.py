...
if __name__ == "__main__":
    ...
    df['Price'] = pd.to_numeric(df['Price'], errors='coerce').apply(lambda x: f"{x:.2f}" if pd.notna(x) else "N/A")
    df['PEG (Fwd P/E to EPS 5Y Groth)'] = df['PEG (Fwd P/E to EPS 5Y Groth)'].apply(lambda x: f"{x:.4f}" if pd.notna(x) else "N/A")
